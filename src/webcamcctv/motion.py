...
class MotionDetector:
    def __init__(self, sensitivity: float, minimum_area: int) -> None:
        self.sensitivity, self.minimum_area = sensitivity, minimum_area
        self.background = cv2.createBackgroundSubtractorMOG2(
            history=400, varThreshold=max(8, 64 * (1 - sensitivity)), detectShadows=True
        )
    ...
    def detect(self, frame: np.ndarray) -> tuple[bool, float]:
        ...
        return score >= self.sensitivity and any(
